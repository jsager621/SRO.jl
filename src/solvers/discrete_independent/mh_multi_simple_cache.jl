function bpso_simple_cache(problem::DiscreteProblem{T}, args::BPSOArgs)::DiscreteSolution where {T}
    return bpso_simple_cache(Xoshiro(), problem, args)
end

function bpso_simple_cache(
    rng::AbstractRNG,
...
function n_thread_evo_simple_cache(problem::DiscreteProblem, n_steps::Int64)::DiscreteSolution
    return n_thread_evo_simple_cache(Xoshiro(), problem, n_steps)
end

function n_thread_evo_simple_cache(
    rng::AbstractRNG,
    problem::DiscreteProblem{T},
    n_steps::Int64,
)::DiscreteSolution where {T}
    ...
    known_combinations = ConcurrentDict{Vector{Bool}, Float64}()
